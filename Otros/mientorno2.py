...
while True:
    ...
    for i in range(0, filas):
        for j in range(0, columnas):
            if not pausa:
                vecinosvivos=juego[(i-1)%filas, (j-1)%columnas] + juego[(i-1)%filas, j] + juego[(i-1)%filas, (j+1)%columnas] +  juego[i, (j-1)%columnas] + juego[i, (j+1)%columnas] +  juego[(i+1)%filas, (j-1)%columnas] + juego[(i+1)%filas, j] + juego[(i+1)%filas, (j+1)%columnas] ##Esta variable calcula el número de vecinos vivos de cada célula
                if  vecinosvivos> 0.0:
                    if juego[i, j]<3.0:
                            juegocopia[i, j]=juegocopia[i, j]+1 #Si una célula muerta tiene exactamente 3 células vecinas vivas, revive
                else: ##Si la célula estaba viva
                    if juego[i, j]>0:
                        juegocopia[i, j]=juegocopia[i, j]-1 #Si una célula viva tiene menos de 2 o más de 3 células vecinas vivas, muere por soledad o por sobrepoblación
            ...
            if juegocopia[i, j] > 0:
                pygame.draw.polygon(screen, (128, 0, 0), poly, 1) ##Si la célula está muerta, los bordes del cuadrado de esa célula se mantendrán de color gris oscuro (el cuadrado de la célula se pinta de blanco)
            else:
    ...
    time.sleep(1) ##Esperamos un tiempo hasta la siguiente iteración. El juego no termina a menos que pulsemos Ctrl+C para parar el programa.
