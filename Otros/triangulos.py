...
if __name__ == "__main__":
    ...
    COLOR_RELLENO = (100, 200, 150) # Rojo para el clickeado

    # Parámetros geométricos
    TAMAÑO_TRIANGULO = 20  # Distancia del centro a sus vértices
    FILAS = 16
    COLUMNAS = 20
    OFFSET_X = 80
    OFFSET_Y = 80
    ...
    # Dibujar Triángulo
        ...
        if juegocopia[f, c]==1:
            ...
            pygame.draw.polygon(pantalla, (128, 128, 128), esquinas_desplazadas)
    ...
    while corriendo:
        ...
        if not pausa:
            for i in range(0, FILAS):
                for j in range(0, COLUMNAS):
                    vecinosvivos=juego[(i-1)%FILAS, j] + juego[i, (j-1)%COLUMNAS] + juego[i, (j+1)%COLUMNAS] +  juego[(i+1)%FILAS, j] ##Esta variable calcula el número de vecinos vivos de cada célula
                    if juego[i, j]==0 and vecinosvivos==2:
                        juegocopia[i, j]=1
                    if juego[i, j]==1 and vecinosvivos<2:
                        juegocopia[i, j]=0
            # Renderizado
            pantalla.fill(COLOR_FONDO)
